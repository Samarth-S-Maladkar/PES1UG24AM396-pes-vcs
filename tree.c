...
int tree_from_index(ObjectID *id_out) {
    ...
    // Temporary structure for subdirectories
    typedef struct {
        char name[256];
        Tree tree;
    } SubDir;
    ...
    for (int i = 0; i < index.count; i++) {
        ...
        if (!slash) {
            ...
            for (int j = 0; j < subdir_count; j++) {
                if (strcmp(subdirs[j].name, dirname) == 0) {
                    found = j;
                    break;
                }
            }
            ...
            if (found == -1) {
                found = subdir_count++;
                strcpy(subdirs[found].name, dirname);
                subdirs[found].tree.count = 0;
            }

            // Add file inside subdir
            TreeEntry *te = &subdirs[found].tree.entries[subdirs[found].tree.count++];
            te->mode = entry->mode;
    ...
    // 3. Write subdirectory trees
    for (int i = 0; i < subdir_count; i++) {
        void *data;
        size_t len;

        if (tree_serialize(&subdirs[i].tree, &data, &len) != 0)
            return -1;
        ...
        TreeEntry *te = &root.entries[root.count++];
        te->mode = MODE_DIR;
        strcpy(te->name, subdirs[i].name);
        te->hash = sub_id;
    }

    // 4. Serialize root tree
    void *data;
    size_t len;
